<template>
  <div>
    <TableList ref="table" title="广告列表" :columns="columns" :getData="list">
      <Button type="primary" slot="head-left" @click="form.is_show=true; form.title='添加广告'"><Icon type="md-add" /> 添加</Button>
    ...
    <Modal :title="form.title" v-model="form.is_show" @on-ok="ok">
      <Form :model="form.data" :label-width="100">
        ...
        <FormItem label="图片">
          ...
          >
            <Button icon="ios-cloud-upload-outline">上传广告</Button>
          </Upload>
        </FormItem>
      </Form>
    </Modal>
...
export default {
  ...
  methods: {
    ...
    async info(id) {
      this.$Spin.show();
      let res = await this.$api.advert.info({ id });
      if (res) {
        this.form.data = res;
      }
      this.$Spin.hide();
